...
class RedisService:
    def __init__(self):
        try:
            ...
            logger.info(f"Redis async client initialized: {settings.redis_url}")
    ...
    async def get_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            data["message_count"] = await self.redis_client.llen(message_key)
            return data
    ...
    async def delete_session(self, session_id: str) -> bool:
        ...
        for list_key in list_keys:
            await self.redis_client.srem(list_key, session_id)
    ...
    async def list_sessions(self, user_id: Optional[str] = None, limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        session_ids = await self.redis_client.smembers(list_key)
    ...
    async def add_message(self, session_id: str, role: str, content: str, 
        ...
        message_key = self._get_message_key(session_id)
        await self.redis_client.lpush(message_key, json.dumps(message))
        await self.redis_client.expire(message_key, settings.redis_message_ttl)
    ...
    async def get_messages(self, session_id: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        message_key = self._get_message_key(session_id)
        messages = await self.redis_client.lrange(message_key, 0, limit - 1)
